...
The objective of this skill is to take a project idea — however raw or complete — and drive it to a state where Planning can begin with confidence. That means `./.artifacts/A_Project.md` is fully populated, feasibility has been assessed, and known unknowns are captured in the artifact system.

Having a full idea of the project as the first step using this skill is NECESSARY before moving on to the actual planning phase where `./.artifacts/B_Architecture.md` will be solidified.
...
- **Assess** — evaluate the current state of `./.artifacts/A_Project.md` and identify any gaps or areas that need further clarification. Focus on the idea itself and not on the implementation details. This is a critical step to ensure we have a clear understanding of the project idea before moving forward. 

- **Complete** — fill gaps through targeted questions. Work through each incomplete or missing information in `./.artifacts/A_Project.md` using the targeted questions. Wait for my answer before moving to the next. Do not invent answers. If I give a vague response, ask a follow-up until the answer is specific enough to write down.
